...
const testInputStrings=['R8,U5,L5,D3','U7,R6,D4,L4'];

// = distance 159
// const testInputStrings=['R75,D30,R83,U83,L12,D49,R71,U7,L72','U62,R66,U55,R34,D71,R55,D58,R83'];

// = distance 135
// const testInputStrings=['R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51','U98,R91,D20,R16,D67,R40,U7,R15,U6,R7'];

if (process.argv[2]==='test') {
    inputArray = testInputStrings;
} else {
    inputArray = inputStrings;
}

const wireRoutes = inputArray.map((wireValues,i) => wireValues.split(','));

let wirePositions=Array.from(Array(2), v => Array.from(Array(), v => 0));

wireRoutes.forEach((route,i) => {
    let currentposition=[0,0];

    wirePositions[i].push(currentposition);
    route.forEach(movement => {
        let count=0;
        switch(movement.slice(0,1)) {
            case 'U':
                for (count=0;count<Number(movement.slice(1));count++) {
...
const intersections = _.uniqWith(intersect(wirePositions[0],wirePositions[1]),compare);

console.log('%O', intersections);

const distances = intersections.map( intersection => {
    return Math.abs(intersection[0]) + Math.abs(intersection[1]);
})

const closestIntersectionDistance = Math.min(...distances);

console.log('Closest distance to origin is %d', closestIntersectionDistance);

// const closestIntersection = intersections[distances.findIndex( x => x === closestIntersectionDistance)];
const intersectionLengths = intersections.map( intersection => {
    return wirePositions[0].findIndex( x => (x[0] === intersection[0] && x[1] === intersection[1])) +
    wirePositions[1].findIndex( x => (x[0] === intersection[0] && x[1] === intersection[1]));
});

console.log('Closest length to origin is %d', Math.min(...intersectionLengths));
