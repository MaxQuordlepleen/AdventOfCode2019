...
if (process.argv[2]==='test') {
    inputArray = testInputString;
} else {
    inputArray = inputString;
}

const inputs = inputArray.split('\n');

let objectList = {};

//root element
// objectList[inputs[0].split(')')[0]] = { point: null };

inputs.forEach( (obj, i) => {

    let [point, sat] = obj.split(')');

    if (point === 'COM') {
        objectList[sat.toString()] = { point: null };
    } else {
        objectList[sat.toString()] = { point: point.toString() };
    }
});

let orbitCount = 0;

Object.keys(objectList).forEach( sat => {

    //something
    let currentInspectedSat = objectList[sat];

    while (currentInspectedSat.point) {
        orbitCount++;
        currentInspectedSat = objectList[currentInspectedSat.point];
    }
});

console.dir({objectList, orbitCount});
...
//find the right node before adding

        function traversalSearch (node, id) {
        console.log('Does %s equal %s?', id, node.id);
            if (id === node.id) {
                console.log('Yes - found it! (it being %O)', node);
                return node;
            }

            node.children.forEach(child => {
                console.log('No - calling traversalSearch with child %O looking for id %s', child, id);
                return traversalSearch(child, id);
            });

            console.log('something else');
        };

        let foundNode = traversalSearch(result, point);
        if (foundNode) {
